...
class TMPacket(GenericPacket):
    ...
    def __init__(self, data, idb=None):
        ...
        if not idb:
            ...
            # self.idb_manager.get_idb(obt=self.data_header.datetime)
...
class GenericTMPacket:
    ...
    def __init__(self, data, idb=None):
        ...
        if isinstance(self.idb, IDBManager):
            idb = self.idb.get_idb('2.26.35')  # get_idb(bt=self.data_header.datetime)
    ...
    def get_idb(self):
        if isinstance(self.idb, IDBManager):
            idb = self.idb.get_idb('2.26.35')  # get_idb(obt=self.data_header.datetime)
