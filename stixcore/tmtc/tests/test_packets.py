...
@pytest.mark.parametrize(
    "class_header", [(SourcePacketHeader, SOURCE_PACKET_HEADER_STRUCTURE), (TMDataHeader, TM_DATA_HEADER_STRUCTURE)]
)
def test_tmtc_headers(class_header):
    ...
    sph = cls(test_binary)
    assert all([getattr(sph, key) == test_values[key] for key in header.keys() if not key.startswith("spare")])
...
def test_tm_packet(idb):
    ...
    test_values = {n: 2 ** int(v.split(":")[-1]) - 1 for n, v in combind_structures.items()}
    test_binary = bitstring.pack(test_fmt, *test_values.values())
    tmtc_packet = TMPacket(test_binary, idb=idb)
    assert all(
        [
            getattr(tmtc_packet.source_packet_header, key) == test_values[key]
    ...
    assert all(
        [
            getattr(tmtc_packet.data_header, key) == test_values[key]
...
def test_tc_packet():
    ...
    assert all(
        [
            getattr(tmtc_packet.data_header, key) == test_values[key]
            for key in TC_DATA_HEADER_STRUCTURE.keys()
            if not key.startswith("spare")
        ]
    )
