...
class FlareListANC(SingleProductProcessingStepMixin):
    ...
    def test_for_processing(self, month: date,
        ...
        """
        try:
            wp = phm.has_processed_fits_products(self.flm.productCls.NAME,
    ...
    def process_fits_files(self, months: list[date], *, soopmanager: SOOPManager,
        ...
        for month in months:
            data, control, energy = self.flm.get_data(start=month,
                                                      end=(month + pd.DateOffset(months=1)).date(),
            ...
            if issubclass(self.flm.productCls, FlarePositionMixin):
                self.flm.productCls.add_flare_position(data, fido_client, filter_function=self.flm.filter_flare_function)  # noqa
            ...
            all_files.extend([SingleProcessingStepResult(anc.name, anc.level, anc.type,
                                                         anc.get_processing_version(),
                                                         fop, str(month), datetime.now())
