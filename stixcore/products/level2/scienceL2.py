...
class Spectrogram(ScienceProduct, L2Mixin):
    ...
    def is_datasource_for(cls, *, service_type, service_subtype, ssid, **kwargs):
        return kwargs["level"] == "L2" and service_type == 21 and service_subtype == 6 and ssid == 24

    def get_additional_extensions(self):
        return [
            (
                self.data_corrected if (hasattr(self, "data_corrected") and len(self.data_corrected) > 0) else None,
                "data_corrected",
            )
        ]
