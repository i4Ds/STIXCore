...
class LightCurve(QLProduct):
    ...
    def from_levelb(cls, levelb, parent="", keep_parse_tree=True):
        ...
        control.add_data("compression_scheme_counts_skm", _get_compression_scheme(packets, "NIX00272"))

        counts_flat = packets.get_value("NIX00272")
        counts_var_flat = packets.get_value("NIX00272", attr="error")

        flat_indices = np.hstack((0, np.cumsum([*control["num_samples"]]) * control["num_energies"])).astype(int)
        ...
        control.add_data("compression_scheme_triggers_skm", _get_compression_scheme(packets, "NIX00274"))

        triggers = packets.get_value("NIX00274").flatten()
        triggers_var = packets.get_value("NIX00274", attr="error").flatten()

        cur_index = 0
        for i, (num, skm) in enumerate(zip(control["num_samples"], control["compression_scheme_triggers_skm"])):
            if skm.tolist() == _SKM_SCALING:
                ...
                triggers[cur_slice], triggers_var[cur_slice] = unscale_triggers(
                    triggers[cur_slice],
                    integration=duration[cur_slice],
                    detector_masks=control["detector_mask"][i : i + 1],
                    ssid=levelb.ssid,
                )
            cur_index += num
    ...
    def is_datasource_for(cls, *, service_type, service_subtype, ssid, **kwargs):
        return kwargs["level"] == "L0" and service_type == 21 and service_subtype == 6 and ssid == 30
...
class Background(QLProduct):
    ...
    def from_levelb(cls, levelb, parent="", keep_parse_tree=True):
        ...
        control.add_data("compression_scheme_counts_skm", _get_compression_scheme(packets, "NIX00278"))
        ...
        for i, (num, skm) in enumerate(zip(control["num_samples"], control["compression_scheme_triggers_skm"])):
            if skm.tolist() == _SKM_SCALING:
                ...
                logger.debug("Unscaling triggers")
                triggers[cur_slice], triggers_var[cur_slice] = unscale_triggers(
                    triggers[cur_slice], integration=duration[cur_slice], detector_masks=dmask, ssid=levelb.ssid
                )
            cur_index += num
...
class Spectra(QLProduct):
    ...
    def from_levelb(cls, levelb, parent="", keep_parse_tree=True):
        ...
        control.add_data("compression_scheme_triggers_skm", _get_compression_scheme(packets, "NIX00484"))
        ...
        # TODO Handle NIX00089 value of zero ie valid packet with no data
        ...
        for i in range(452, 484):
            counts.append(packets.get_value(f"NIX00{i}"))
            counts_var.append(packets.get_value(f"NIX00{i}", attr="error"))
        ...
        for i, (num, skm) in enumerate(zip(control["num_samples"], control["compression_scheme_triggers_skm"])):
            if skm.tolist() == _SKM_SCALING:
                ...
                logger.debug("Unscaling triggers")
                triggers[cur_slice], triggers_var[cur_slice] = unscale_triggers(
                    triggers[cur_slice], integration=duration[i][0], detector_masks=dmask, ssid=levelb.ssid
                )
            cur_index += num

        detector_index = np.pad(np.array(did, np.int16), (pad_before, pad_after), mode="edge")
        num_integrations = np.pad(
            np.array(packets.get_value("NIX00485"), np.uint16), (pad_before, pad_after), mode="edge"
        )
    ...
    def _get_time(cls, control, num_energies, packets, pad_before, pad_after):
        ...
        duration = SCETimeDelta(
            np.pad(np.hstack(durations), (pad_before, pad_after), mode="edge").reshape(-1, num_energies)
        )

        scet_timerange = SCETimeRange(start=time[0, 0] - duration[0, 0] / 2, end=time[-1, -1] + duration[-1, 0] / 2)

        return duration, time, scet_timerange

    @classmethod
    def is_datasource_for(cls, *, service_type, service_subtype, ssid, **kwargs):
        return kwargs["level"] == "L0" and service_type == 21 and service_subtype == 6 and ssid == 32
...
class Variance(QLProduct):
    ...
    def from_levelb(cls, levelb, parent="", keep_parse_tree=True):
        ...
        control.add_data("compression_scheme_variance_skm", _get_compression_scheme(packets, "NIX00281"))
    ...
    @classmethod
    def is_datasource_for(cls, *, service_type, service_subtype, ssid, **kwargs):
        return kwargs["level"] == "L0" and service_type == 21 and service_subtype == 6 and ssid == 33
...
class FlareFlag(QLProduct):
    ...
    @classmethod
    def is_datasource_for(cls, *, service_type, service_subtype, ssid, **kwargs):
        return kwargs["level"] == "L0" and service_type == 21 and service_subtype == 6 and ssid == 34
...
class EnergyCalibration(QLProduct):
    ...
    def from_levelb(cls, levelb, parent="", keep_parse_tree=True):
        ...
        control.add_data("compression_scheme_counts_skm", _get_compression_scheme(packets, "NIX00158"))

        subspec_data = {}
        j = 129
        for subspec, i in enumerate(range(300, 308)):
            subspec_data[subspec + 1] = {
                "num_points": packets.get_value(f"NIXD0{j}"),
        ...
        control["subspec_num_points"] = (np.vstack([v["num_points"] for v in subspec_data.values()]).T + 1).astype(
            np.uint16
        )
        control["subspec_num_summed_channel"] = (
            np.vstack([v["num_summed_channel"] for v in subspec_data.values()]).T + 1
        ).astype(np.uint16)
        control["subspec_lowest_channel"] = (np.vstack([v["lowest_channel"] for v in subspec_data.values()]).T).astype(
            np.uint16
        )
        ...
        for i, subspectrum_mask in enumerate(control["subspectrum_mask"]):
            ...
            sub_channels = [
                np.arange(control["subspec_num_points"][i, index]) * (control["subspec_num_summed_channel"][i, index])
                + control["subspec_lowest_channel"][i, index]
        ...
        tids = np.hstack([[unique_times_lookup[t.as_float()]] * n for t, n in zip(time, control["num_samples"])])
        c_in = list(chain.from_iterable([repeat(c, n) for c, n in zip(channels, control["num_samples"])]))
    ...
    @classmethod
    def is_datasource_for(cls, *, service_type, service_subtype, ssid, **kwargs):
        return kwargs["level"] == "L0" and service_type == 21 and service_subtype == 6 and ssid == 41
...
class TMStatusFlareList(QLProduct):
    ...
    def from_levelb(cls, levelb, parent="", keep_parse_tree=True):
        ...
        if tmp["num_flares"].sum() > 0:
            ...
            flares.add_basic(name="highest_flareflag", nix="NIX00289", packets=packets, dtype=np.byte)
    ...
    @classmethod
    def is_datasource_for(cls, *, service_type, service_subtype, ssid, **kwargs):
        return kwargs["level"] == "L0" and service_type == 21 and service_subtype == 6 and ssid == 43
