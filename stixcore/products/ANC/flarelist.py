...
def make_stix_fitswcs_header(data, flare_position, *, scale, exposure, rotation_angle,
                             energy_range):
    """Create a FITS WCS header for the given data."""
    header = make_fitswcs_header(data, flare_position,
                                 telescope="STIX",
...
class FlarePositionMixin:
    ...
    """
    @classmethod
    def add_flare_position(cls, data, fido_client: STIXClient, *,
        ...
        data['cpd_path'] = Column(" " * 500, dtype=str,
                                  description='TDB')
        data['_position_status'] = Column(False, dtype=bool,
                                          description='TDB')
        data['_position_message'] = Column(" " * 500, dtype=str,
                                           description='TDB')
        to_remove = []
        pass_filter = 0
        no_ephemeris = 0
        ...
        for i, row in enumerate(data):
            if filter_function(row):
                ...
                if day in day_asp_ephemeris_cache:
                    ...
                    anc_res = fido_client.search(a.Time(peak_time, peak_time),
                                                 a.Instrument.stix,
                ...
                if len(anc_res) < 1:
                    logger.warning(f'No ephemeris data found for flare at time '
                ...
                cpd_res = fido_client.search(a.Time(start_time, end_time),
                                             a.Instrument.stix,
                ...
                if len(cpd_res) > 1:
                    ...
                    cpd_res["inc_peak"] = np.logical_and(peak_time >= cpd_res["Start Time"],
                                                         peak_time <= cpd_res["End Time"])
                    ...
                    for i, path in enumerate(cpd_res["path"]):
                        ...
                        cpd_res["eend"][i] = energies["e_high"][-1].value if len(energies) < 31 \
                            else energies["e_high"][-2].value
        ...
        logger.info(f'Flare position calculated for month {month} with {total_flares} flares, '
...
class FlarePeekPreviewMixin:
    ...
    def add_peek_preview(cls, data, energies, parent, fido_client: STIXClient, img_processor, *,
        ...
        data['peek_preview_path'] = Column(" " * 500, dtype=str,
                                           description='TDB')
        data['preview_start_UTC'] = [Time(d, format='isot', scale='utc')
                                     for d in data[peek_time_colname]]
        data['preview_end_UTC'] = [Time(d, format='isot', scale='utc')
                                   for d in data[peek_time_colname]]
        data['_peek_preview_status'] = Column(False, dtype=bool,
                                              description='TDB')
        data['_peek_preview_message'] = Column(" " * 500, dtype=str,
                                               description='TDB')
        to_remove = []
        products = []
        images = 0
            ...
            if aux_ephemeris_path.exists() and cpd_path.exists():
                try:
                    ...
                    for energy_range in [[4, 18], [18, 120]] * u.keV:
                        ...
                        isc_10_3 = [3, 20, 22, 16, 14, 32, 21, 26, 4, 24, 8, 28, 15, 27,
                                    31, 6, 30, 2, 25, 5, 23, 7, 29, 1]
                        ...
                        clean_map, model_map, resid_map = vis_clean(
                            vis10_3, imsize, pixel_size=pixel, gain=0.1, niter=200,
                            clean_beam_width=20 * u.arcsec)
                        comments.append(f"clean map with {len(col_idx)} visibilities")
                        comments.append(f"clean gain: {0.1}, niter: {200}, "
                                        f"clean beam width: {20 * u.arcsec}")
                        ...
                        fp_hp = flare_position.transform_to(Helioprojective(obstime=vis_tr.center,
                                                                            observer=solo))
                        header = make_stix_fitswcs_header(map_with_erange, fp_hp,
                                                          scale=pixel,
        ...
        logger.info(f'Flare images created for month {month} with {len(data)} flares, '
...
class FlarelistSDCLoc(FlarelistSDC, FlarePositionMixin):
    ...
    def __init__(self, *, service_type=0, service_subtype=0, ssid=3, data, month, **kwargs):
        ...
        super().__init__(service_type=0, service_subtype=0, ssid=3,
...
class FlarelistSDCLocImg(FlarelistSDCLoc, FlarePeekPreviewMixin):
    ...
    def __init__(self, *, service_type=0, service_subtype=0, ssid=4, data, month, **kwargs):
        ...
        super().__init__(service_type=0, service_subtype=0, ssid=4,
                         data=data, month=month, **kwargs)
