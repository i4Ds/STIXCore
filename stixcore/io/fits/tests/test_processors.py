...
def test_levelb_processor_generate_primary_header(datetime, product):
    ...
    dummy_control_data = {"scet_coarse": [beg.coarse, end.coarse], "scet_fine": [beg.fine, end.fine]}
...
def test_level0_processor_generate_filename():
    ...
    with patch("stixcore.products.level0.scienceL0.ScienceProduct") as product:
        ...
        filename = processor.generate_filename(product, version=1)
        assert filename == "solo_L0_stix-sci-a-name_0000012345-0000098765_V01_0000123456.fits"

        product.control.colnames = ["request_id", "tc_packet_seq_control"]
        filename = processor.generate_filename(product, version=1)
        assert filename == "solo_L0_stix-sci-a-name_0000012345-0000098765_V01_0000123456-98765.fits"
...
@pytest.mark.parametrize(
    "p_file",
    [test_data.products.L0_LightCurve_fits[0], test_data.products.L1_LightCurve_fits[0]],
    ids=["ql_l0", "ql_l1"],
)
