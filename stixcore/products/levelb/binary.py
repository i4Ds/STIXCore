...
class LevelB(BaseProduct):
    ...
    def __repr__(self):
        return (
            f"<LevelB {self.level}, {self.service_type}, {self.service_subtype}, {self.ssid} "
    ...
    def __add__(self, other):
        ...
        if np.abs([((len(data["data"][i]) / 2) - (control["data_length"][i] + 7)) for i in range(len(data))]).sum() > 0:
    ...
    def from_tm(cls, tmfile):
        ...
        for prod_key, packets in packet_data.items():
            ...
            for packet in packets:
                ...
                dh.pop("datetime")
                headers.append(
                    {**sh, **dh, "raw_file": packet.source[0], "packet": packet.source[1], "request_id": rid}
                )
            ...
            if ssid is not None:
                control["ssid"] = ssid
            product = LevelB(
                service_type=service_type, service_subtype=service_subtype, ssid=ssid, control=control, data=data
            )
            yield product
    ...
    def extract_sequences(self):
        ...
        if self.service_type == 3 and self.service_subtype == 25 and self.ssid in {1, 2}:
            return [self], []
        elif self.service_type == 21 and self.service_subtype == 6 and self.ssid in {30, 31, 32, 33, 34, 43}:
            return [self], []
        ...
        for seq in sequences:
            if len(seq) == 1 and flags[seq[0]] == SequenceFlag.STANDALONE:
                ...
                flags[seq[0]] == SequenceFlag.FIRST
                and flags[seq[-1]] == SequenceFlag.LAST
                and (
                    (
                        (
                            self.control["sequence_count"][seq[-1]]
                            + (  # possible role over off 14bit psc value
                                0
                                if self.control["sequence_count"][seq[-1]] > self.control["sequence_count"][seq[0]]
                                else max(self.control["sequence_count"][seq])
                            )
                        )
                        - self.control["sequence_count"][seq[0]]
                        + 1
                    )
                    >= len(seq) - 1
                )
                ...
                s_rep = "\n".join(self[seq].control["sequence_count", "sequence_flag", "scet_coarse"].pformat_all())
