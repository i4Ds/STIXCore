...
ENERGY_CHANNELS = read_energy_channels(
    Path(__file__).parent.parent.parent / "config" / "data" / "common" / "detector" / "ScienceEnergyChannels_1000.csv"
)
...
def fix_detector_mask(control, detector_mask):
    ...
    For a time the BKG and CFL event were not recorded so even if requested will be missing which
    ...
    """
    if not np.any(
        (control["time_stamp"].coarse >= SUM_DMASK_SCET_COARSE_RANGE[0])
...
class ScienceProduct(CountDataMixin, GenericProduct, EnergyChannelsMixin, FitsHeaderMixin):
    ...
    def __init__(self, *, service_type, service_subtype, ssid, control, data, **kwargs):
        ...
        """
        super().__init__(
            service_type=service_type, service_subtype=service_subtype, ssid=ssid, control=control, data=data, **kwargs
        )
...
class RawPixelData(ScienceProduct):
    ...
    def from_levelb(cls, levelb, parent="", keep_parse_tree=True):
        ...
        dd = [
            (tmp["pixel_id"][s:e], tmp["detector_id"][s:e], tmp["channel"][s:e], counts_1d[s:e])
    ...
    def is_datasource_for(cls, *, service_type, service_subtype, ssid, **kwargs):
        return kwargs["level"] == "L0" and service_type == 21 and service_subtype == 6 and ssid == 20
...
class CompressedPixelData(ScienceProduct):
    ...
    def from_levelb(cls, levelb, parent="", keep_parse_tree=True):
        ...
        if np.unique(t_skm, axis=0).shape[0] != 1:
            ...
            header_comments.append("Multiple compression schemes detected, trigger values maybe incorrect")
        ...
        data.add_basic(name="rcr", nix="NIX00401", attr="value", packets=packets, dtype=np.ubyte)
        data.add_basic(name="num_pixel_sets", nix="NIX00442", attr="value", packets=packets, dtype=np.ubyte)
        ...
        if cls is CompressedPixelData:
            ...
            pixel_masks = np.array([list(map(int, format(pm, "012b")))[::-1] for pm in pixel_mask_ints])
        ...
        triggers_var = np.array([packets.get_value(f"NIX00{i}", attr="error") for i in range(242, 258)])
        ...
        if control["compression_scheme_triggers_skm"].tolist() == [[0, 0, 7]]:
            ...
            triggers, triggers_var = unscale_triggers(
                triggers,
                integration=data["integration_time"],
        ...
        if cls is CompressedPixelData:
            ...
            for i, nc in enumerate(tmp["num_data_elements"]):
                ...
                if cur_counts.shape[1] != 12:
                    ...
                    try:
                        ...
                        last_match_index = np.where(data["pixel_masks"].sum(axis=1) == cur_counts.shape[1])
            ...
            counts = np.array(counts).reshape(
                unique_times.size, unique_energies_low.size, data["detector_masks"].sum(axis=1).max(), 12
            )
            counts_var = np.array(counts_var).reshape(
                unique_times.size, unique_energies_low.size, data["detector_masks"].sum(axis=1).max(), 12
            )
        elif cls is SummedPixelData:
            ...
            counts_var = counts_var_flat.reshape(
                unique_times.size,
                unique_energies_low.size,
                data["detector_masks"].sum(axis=1).max(),
                data["num_pixel_sets"][0].sum(),
        ...
        # No longer re-binning back to science energy channels leaving here for in case
    ...
    @classmethod
    def is_datasource_for(cls, *, service_type, service_subtype, ssid, **kwargs):
        return kwargs["level"] == "L0" and service_type == 21 and service_subtype == 6 and ssid == 21
...
class SummedPixelData(CompressedPixelData):
    ...
    @classmethod
    def is_datasource_for(cls, *, service_type, service_subtype, ssid, **kwargs):
        return kwargs["level"] == "L0" and service_type == 21 and service_subtype == 6 and ssid == 22
...
class Visibility(ScienceProduct):
    ...
    def from_levelb(cls, levelb, parent="", keep_parse_tree=True):
        ...
        if control["compression_scheme_triggers_skm"].tolist() == [[0, 0, 7]]:
            ...
            triggers, triggers_var = unscale_triggers(
                triggers,
                integration=SCETimeDelta(data["integration_time"]),
                detector_masks=data["detector_masks"],
                ssid=levelb.ssid,
                factor=factor,
            )
        ...
        data["detector_id"] = np.array(packets.get_value("NIX00100")).reshape(unique_times.size, -1, num_detectors)
        ...
        data["real_comp_err"] = np.sqrt(
            packets.get_value("NIX00263", attr="error").reshape(unique_times.size, num_detectors, -1)
        )
        data.add_meta(name="real_comp_err", nix="NIX00263", packets=packets)
        data["imaginary"] = packets.get_value("NIX00264").reshape(unique_times.size, num_detectors, -1)
        data.add_meta(name="imaginary", nix="NIX00264", packets=packets)
        data["imaginary_comp_err"] = np.sqrt(
            packets.get_value("NIX00264", attr="error").reshape(unique_times.size, num_detectors, -1)
        )
        data.add_meta(name="imaginary", nix="NIX00264", packets=packets)
    ...
    @classmethod
    def is_datasource_for(cls, *, service_type, service_subtype, ssid, **kwargs):
        return kwargs["level"] == "L0" and service_type == 21 and service_subtype == 6 and ssid == 23
...
class Spectrogram(ScienceProduct):
    ...
    def from_levelb(cls, levelb, parent="", keep_parse_tree=True):
        ...
        cids = [np.arange(emin, emax + 1, eunit) for (emin, emax, eunit) in zip(e_min, e_max, energy_unit)]
        ...
        pixel_masks_expanded = [np.repeat(pm.reshape(-1, 1), n, 1) for pm, n in zip(pixel_masks_orig[0], num_times)]
        ...
        if control["compression_scheme_triggers_skm"].tolist() == [[0, 0, 7]]:
            ...
            logger.debug(f"Unscaling trigger: {factor}")
            triggers, triggers_var = unscale_triggers(
                triggers, integration=deltas, detector_masks=control["detector_masks"], ssid=levelb.ssid, factor=factor
            )
    ...
    @classmethod
    def is_datasource_for(cls, *, service_type, service_subtype, ssid, **kwargs):
        return kwargs["level"] == "L0" and service_type == 21 and service_subtype == 6 and ssid == 24
...
class Aspect(ScienceProduct):
    ...
    def from_levelb(cls, levelb, parent="", keep_parse_tree=True):
        ...
        offsets = SCETimeDelta(np.concatenate([delta_time[i] * np.arange(0, ns) for i, ns in enumerate(samples)]))
        timedel = SCETimeDelta(np.concatenate([delta_time[i] * np.ones(ns) for i, ns in enumerate(samples)]))
    ...
    @classmethod
    def is_datasource_for(cls, *, service_type, service_subtype, ssid, **kwargs):
        return kwargs["level"] == "L0" and service_type == 21 and service_subtype == 6 and ssid == 42
