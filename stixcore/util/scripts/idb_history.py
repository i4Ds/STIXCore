...
if __name__ == "__main__":
    idbs = {167: "2.26.31", 174: "2.26.32", 178: "2.26.33", 179: "2.26.34", 181: "2.26.35", 183: "2.26.36"}
    ...
    def extract_time_swver(file):
        print(file)
        hdul = fits.open(file)
        times = SCETime.from_float(hdul[0].header.get("OBT_BEG") * u.s) + (hdul[2].data["time"] * u.cs)
        sw_ver = hdul[2].data["sw_version_number"]
        idpu_identifier = [0 if idpu == "Nominal" else 1 for idpu in hdul[2].data["idpu_identifier"]]
        return times, sw_ver, idpu_identifier

    files = [Path(f) for f in glob.glob(str(fitsdir / "L1/**/solo_L1_stix-hk-maxi*.fits"), recursive=True)]
    ...
    for i, row in enumerate(table):
        ...
        p["validityPeriodUTC"] = (
            [
                start_time.to_datetime().isoformat(timespec="milliseconds"),
                end_time.to_datetime().isoformat(timespec="milliseconds"),
            ],
        )
        p["validityPeriodOBT"] = [
            {"coarse": int(start_time.coarse), "fine": int(start_time.fine)},
            {"coarse": int(end_time.coarse), "fine": 0},
        ]
        periods.append(p)
        start_time = end_time
    ...
    p["validityPeriodUTC"] = (
        [start_time.to_datetime().isoformat(timespec="milliseconds"), "2136-02-07T08:11:44.778+00:00"],
    )
    p["validityPeriodOBT"] = [
        {"coarse": int(start_time.coarse), "fine": int(start_time.fine)},
        {"coarse": int(end_time.coarse), "fine": 0},
